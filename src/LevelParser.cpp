...
void CLevelReader::Characters(const UGKS_String &rText, DWORD dwAppData, bool &bAbort)
{
	...
	case MESH_L:
		...
		case CHARS_AMANDA:
			msg = rText;
			if (msg.compare(defaultAmanda->GetMeshName()))
			{		//return 0 if strings are equal
				int ind = MeshesManager.AddModel(msg);
				Amanda->IndMesh = ind;
				Amanda->Mesh = MeshesManager.GetMesh(Amanda->IndMesh);
				Amanda->SetMeshName(Amanda->Mesh->GetFileName());
			}
			else
			{
				Amanda->IndMesh = defaultAmanda->IndMesh;
				Amanda->Mesh = defaultAmanda->Mesh;
				Amanda->SetMeshName(defaultAmanda->GetMeshName());
			}
			break;
		case CHARS_NAVY:
	...
	case TEXTURE2D_L:
		...
		case CHARACTER_L:
			...
			{
			case CHARS_AMANDA:
				if (rText.compare(TexturesManager.Textures[defaultAmanda->IndTexture2D]->GetFileName()))
					Amanda->IndTexture2D = TexturesManager.CreateTexture(rText);
				break;
			case CHARS_BONUS:
				break;
			case CHARS_BRICK:
	...
	case TEXTURE3D_L:
		if (CHARACTER_L == SubTop())
			...
			{
			case CHARS_AMANDA:
				if (rText.compare(TexturesManager.Textures[defaultAmanda->IndTexture3D]->GetFileName()))
					Amanda->IndTexture3D = TexturesManager.CreateTexture(rText);
				break;
			case CHARS_BONUS:
				break;
			case CHARS_BRICK:
	...
	case TIMEBONUS_L:
		...
		break;
	case TIMEAMANDA_L:
		Game->DefaultUpdPeriod[CHARS_AMANDA] = atof(UGKS_string2charstr(rText));
		break;
	case TIMECIRCLESHIP_L:
	...
	case X_L:
		...
		case UGKPHY_TRASLATION:
			...
			{
			case CHARS_AMANDA:
				Amanda->MoveTo(f, Amanda->Position.v[YDIM], Amanda->Position.v[ZDIM]);
				break;
			case CHARS_BUNKER:
		...
		case UGKPHY_SIZE:
			...
			{
			case CHARS_AMANDA:
				Amanda->CharAABB.AABB[CHAR_BBSIZE][XDIM].Value = f;
				break;
			case CHARS_SHIP:
	...
	case Y_L:
		...
		case UGKPHY_TRASLATION:
			...
			{
			case CHARS_AMANDA:
				Amanda->MoveTo(Amanda->Position.v[XDIM], f, Amanda->Position.v[ZDIM]);
				break;
			case CHARS_BUNKER:
		...
		case UGKPHY_SIZE:
			...
			{
			case CHARS_AMANDA:
				Amanda->CharAABB.AABB[CHAR_BBSIZE][YDIM].Value = f;
				break;
			case CHARS_SHIP:
	...
	case Z_L:
		...
		case UGKPHY_TRASLATION:
			...
			{
			case CHARS_AMANDA:
				Amanda->MoveTo(Amanda->Position.v[XDIM], Amanda->Position.v[YDIM], f);
				break;
			case CHARS_BUNKER:
		...
		case UGKPHY_SIZE:
			...
			{
			case CHARS_AMANDA:
				Amanda->CharAABB.AABB[CHAR_BBSIZE][ZDIM].Value = f;
				break;
			case CHARS_SHIP:
