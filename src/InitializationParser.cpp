...
void CInitializationParser::InitializeDefaults()
{
	...
	//AMANDA
	defaultAmanda->Explosion.SubType = CE_SHIP_EXPLOSION;
	//NAVY
	AssignTMG(Navy);
	//SHIP
	...
	switch(StateStackTop)
	{
	 ...
	 case ANIMATION2D:
			if (StateStackTop == Tag)
			{
				...
				{
				case CHARS_AMANDA:
					defaultAmanda->IndAnimation2D = AnimationsManager.Animations.size() - 1;
					break;
				case CHARS_BONUS:
						break;
				case CHARS_BRICK:
...
CTextureAnimation *aniAux;
	...
	case EXPLOSION:
			...
			{
			case CHARS_AMANDA:
				defaultAmanda->Hit_duration = atof(UGKS_string2charstr(rText));
				break;
			case CHARS_BONUS:
	...
	case HEALTH:	//How strong is the character. How much has to be hurt before dieing
			...
			{
			case CHARS_AMANDA:
				defaultAmanda->Health = atof(UGKS_string2charstr(rText));
				break;
			case CHARS_BONUS:
	...
	case MESH:
			...
			{
			case CHARS_AMANDA:
				if (MeshSemaphore[CHARS_AMANDA])
				{
					MeshSemaphore[CHARS_AMANDA] = false;
					int ind = MeshesManager.AddModel(rText);
					defaultAmanda->IndMesh = ind;
					defaultAmanda->Mesh = MeshesManager.GetMesh(ind);
					defaultAmanda->SetMeshName(defaultAmanda->Mesh->GetFileName());
				}
				break;
			case CHARS_BONUS:
				 break;
	...
	case TEXTURE2D:
		...
		case CHARACTER:
			switch (CharType)
				{
				case CHARS_AMANDA:
					defaultAmanda->IndTexture2D = TexturesManager.CreateTexture(rText);
					break;
				case CHARS_BONUS:
	...
	case TEXTURE3D:
		switch (CharType)
			{
			case CHARS_AMANDA:
				defaultAmanda->IndTexture3D = TexturesManager.CreateTexture(rText);
				break;
			case CHARS_BONUS:
				 break;
			case CHARS_BRICK:
